    ...
    def get(self, request):
        ...
        if area_id is None:
            ...
            if provinces_list is None:
                # 获取所有省的数据
                provinces_model_qs = Areas.objects.filter(parent=None)
            ...
            area_datas = cache.get('area_datas_%s' % area_id)
            if area_datas is None:
                try:
                    area_model = Areas.objects.get(id=area_id)  # 此模型有可能是某个省,也有可能是某个市
