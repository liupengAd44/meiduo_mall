...
class OrdersView(LoginRequiredView):
    def get(self, request):
        ...
        skus = SKU.objects.filter(id__in=redis_select)  # ORM跨数据类型查询,数据类型自动转换


        total_count = 0
        total_amount = Decimal('0.00')
        for sku in skus:
            sku.count = int(redis_carts[str(sku.id).encode()])

            sku.amount = int(sku.price)*sku.count

            total_count += sku.count  # 累加数量
            total_amount += sku.amount  # 累加总价
            ...
            try:
                ...
                for sku in skus:
                    while True:
                        ...
                        # 获取当前商品要购买的数量
                        buy_count = int(redis_cart[str(sku.id).encode()])
                        # 获取当前商品的库存和销量
